...
class MessageModel(db.Model):
    ...
    def find_conversation_addressees(cls, login):
        ...
        query_base = cls.query.with_entities(case([cls.receiver == login, cls.sender], cls.receiver).label("receiver"), func.max(cls.sentAt)).filter_by(or_(receiver=login, sender=login)).group_by(cls.receiver).order_by(func.max(cls.sentAt).desc()).all()
