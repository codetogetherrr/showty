...
class MessageModel(db.Model):
    ...
    def find_conversation_addressees(cls, login):
        ...
        query_base = cls.query.filter(or_(cls.receiver==login, cls.sender==login)).with_entities(case([(cls.receiver == login, cls.sender)], else_ = cls.receiver).label("receiver"), func.max(cls.sentAt)).group_by(cls.sender).order_by(func.max(cls.sentAt).desc()).all()
